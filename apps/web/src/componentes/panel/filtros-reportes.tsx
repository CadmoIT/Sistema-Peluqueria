...
export function FiltrosReportes({
  ...
  const router = useRouter(),
    ...
    [pendiente, iniciar] = useTransition();
  function cambiar(campo: string, valor: string) {
    const query = new URLSearchParams(params.toString());
    if (valor) query.set(campo, valor);
    else query.delete(campo);
    iniciar(() =>
      router.replace(`/panel/reportes?${query}`, { scroll: false }),
    );
  }
  return (
    <>
      <div
        className="herramientas-modulo reportes-filtros"
        aria-busy={pendiente}
      >
        ...
        {sedes.length > 1 && (
          <label className="filtro-discreto">
            Local
            <ControlFiltroLocal
              ariaLabel="Local del reporte"
              sedes={sedes}
              valor={local}
              onChange={(valor) => cambiar("local", valor)}
            />
          </label>
        )}
        <label className="filtro-discreto">
      ...
      {pendiente && (
        <div className="reportes-cargando" role="status">
          <span className="solo-lectores">Actualizando reporte</span>
